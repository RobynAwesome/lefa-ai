...
async def test_voice_rtc_loop():
    with TemporaryDirectory() as tmpdir:
        ...
        obs_id = await stt.recognize_speech(
            audio_source="mic_1", mock_transcript="Buy 10 shares of AAPL"
        )
        assert obs_id is not None

        # Test TTS (The Voice -> The Face/Ears)
        tts = VoiceProjector(ark_ledger=ark)
        rev_id = await tts.speak_reveal(
            context_id=obs_id, spoken_text="Trade executed successfully."
        )
