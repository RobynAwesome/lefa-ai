...
export const CompanionAvatar: React.FC<CompanionAvatarProps> = ({
  ...
  // Vite publicDir points at the repository-governed /assets directory.
  const imageSrc = '/companion/lefa-companion-root.jpg';
      ...
      <div className={`relative ${sizeClasses[size]} flex items-center justify-center p-3`}>
        <div
          className={`absolute inset-0 rounded-full transition-opacity duration-700 pointer-events-none ${
            state === 'observing'
              ? 'opacity-70 bg-gradient-to-r from-[#d4af37]/15 via-transparent to-[#c5a059]/15'
              : state === 'ledgered'
                ? 'opacity-90 bg-gradient-to-tr from-[#e5c158]/20 via-transparent to-[#d4af37]/20'
                : state === 'hold'
                  ? 'opacity-60 bg-gradient-to-br from-[#d97706]/20 via-transparent to-[#b45309]/15'
                  : state === 'reveal'
                    ? 'opacity-85 bg-gradient-to-t from-[#fef08a]/20 via-[#d4af37]/15 to-transparent'
        ...
        <div className={`relative w-full h-full rounded-full overflow-hidden border-2 transition-all duration-700 z-10 ${getStateAura()}`}>
          <img
            src={imageSrc}
      ...
      {showKaomojiBadge && (
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#121216]/90 border border-[#d4af37]/30 backdrop-blur-md shadow-lg text-xs font-mono text-[#f4f4f5] hover:border-[#d4af37] transition-colors">
            <span className="text-[#e5c158] font-bold">{getKaomojiForState()}</span>
          </div>
        </motion.div>
      )}
    </div>
